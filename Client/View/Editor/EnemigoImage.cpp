...
EnemigoImage::EnemigoImage(int horda, SDL_Texture *texture, Renderer &renderer) : horda(horda),
                                                                                  Image(0, 0,
...
void EnemigoImage::draw(int number, int padding) {
    ...
    button.y = HORDA_ENEMIGO_PADDING + (hordaNro.quot - padding) * HORDA_TOTAL_HEIGHT;
    Image::draw(number, 0);
}
